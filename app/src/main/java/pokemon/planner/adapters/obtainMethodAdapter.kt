...
class obtainMethodAdapter(private val context: Context, private val pokemon: Pokemon, private val team: Team): RecyclerView.Adapter<obtainMethodAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if(pokemon.evolvesFrom.size > 0){
            if(Pokedex.pokemonAvailability[pokemon.evolvesFrom.get(0)-1][team.version.pokemonList].equals("B") || Pokedex.pokemonAvailability[pokemon.evolvesFrom.get(0)-1][team.version.pokemonList].equals("U")){
                var encounters = Pokedex.encounters[team.version.versionId-1][Integer.parseInt(pokemon.number)-1]
    ...
    private fun getObtainMethod(): String {
        ...
        if(availability.equals("C")){
            return " can be caught in the wild"
        }else if(availability.equals("R")){
            return " is received from an NPC"
