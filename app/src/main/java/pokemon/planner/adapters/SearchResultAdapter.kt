...
class SearchResultAdapter(private val context: Context, private val pokedex: ArrayList<Pokemon>): RecyclerView.Adapter<SearchResultAdapter.ViewHolder>(){
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int){
        ...
        if(pokedex.get(position).typeSecondary.equals(TYPE.NONE)){
            //only has one type
            holder.card1.visibility = View.GONE
            holder.type2.setText(pokedex.get(position).typePrimary.toString().substring(0, 3))
            val background = GradientDrawable()
            background.setStroke(5, Color.GRAY)
            background.cornerRadius = 8f
            background.setColor(Color.parseColor(pokedex.get(position).typePrimary.color))
            holder.card2.background = background
        }else{
            //two types
            holder.card1.visibility = View.VISIBLE
            holder.card2.visibility = View.VISIBLE
            holder.type1.setText(pokedex.get(position).typePrimary.toString().substring(0, 3))
            holder.type2.setText(pokedex.get(position).typeSecondary.toString().substring(0, 3))

            val background1 = GradientDrawable()
            background1.setStroke(5, Color.GRAY)
            background1.cornerRadius = 8f
            background1.setColor(Color.parseColor(pokedex.get(position).typePrimary.color))
            ...
            background2.setColor(Color.parseColor(pokedex.get(position).typeSecondary.color))
